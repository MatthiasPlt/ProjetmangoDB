...
try:
    ...
    if stats_resp.status_code == 200:
        ...
        kpi3.metric(label=" Distance Totale Parcourue", value=f"{stats.get('total_distance', 0):,.2f} km")
except Exception as e:
    st.error("Impossible de charger les statistiques globales.")

st.markdown("---")

st.subheader(" Performance Commerciale par Ville")
try:
...
with col1:
    st.subheader(" Impact du trafic sur la durée de livraison")
    try:
